...
class FLTrainer:
    def __init__(self, conf, logger):
        ...
        if ('defenseTechnique' in conf and conf['defenseTechnique'] is not None):
            ...
            if conf["defenseTechnique"] == "weakDp":
        ...
        if (not self.backdoor is None):
            ...
            if conf["arch"] == "textCC":
                ...
                def collate_batch(batch):
                    ...
                    return label_list.to(conf["device"]), text_list.to(conf["device"]), offsets.to(conf["device"])

                self.backdoorTrainLoader = DataLoader(self.backdoorTrainData,
                                                      batch_size=conf['attackerTrainConfig']['batchSize'], shuffle=True,
                                                      collate_fn=collate_batch)
                self.backdoorTestLoader = DataLoader(self.backdoorTestData,
                                                     batch_size=self.conf['attackerTrainConfig']['batchSize'],
                                                     collate_fn=collate_batch)
            else:
                self.backdoorTrainLoader = DataLoader(self.backdoorTrainData,
                ...
                self.backdoorTestLoader = DataLoader(self.backdoorTestData,
                                                     batch_size=conf['attackerTrainConfig']['testBatchSize'],
                                                     num_workers=1)
        ...
        if ('partitioning' in conf and conf['partitioning'] is not None):
            ...
            self.dataset.partitionTrainData(conf['partitioning'], self.totalUsers, conf)
        ...
        self.globalModel.createDataLoaders(trainData=trainData_u0, testData=self.dataset.testData, config=self.conf,
    ...
    def trainOneEpoch(self, flEpoch):
        ...
        attack = self.attackFreq is not None and (
                    flEpoch - self.attackFromEpoch) % self.attackFreq == 0 and flEpoch >= self.attackFromEpoch
        ...
        for idx in range(len(workers)):
            ...
            localModel.createDataLoaders(trainData=lstWorkerData[idx], testData=self.dataset.testData,
                                         config=self.conf, vocab=self.dataset.vocab)
    ...
    def trainNEpochs(self):
        ...
        for epoch in range(self.startFlEpoch + 1, self.startFlEpoch + self.conf['numFLEpochs'] + 1):
            ...
            if self.defense_technique == "weakDp":
                with torch.no_grad():
                    for param in self.globalModel.model.parameters():
                        param.data.add_(torch.normal(0, 1 * self.epsilon, param.shape).to(self.device))
